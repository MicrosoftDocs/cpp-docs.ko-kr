---
description: '자세히 알아보기: CMDIChildWndEx 클래스'
...
ms.locfileid: "97207724"
---
# <a name="cmdichildwndex-class"></a>CMDIChildWndEx 클래스

`CMDIChildWndEx`클래스는 WINDOWS MDI (다중 문서 인터페이스) 자식 창의 기능을 제공 합니다. [CMDIChildWnd 클래스](../../mfc/reference/cmdichildwnd-class.md)의 기능을 확장 합니다. MDI 애플리케이션에서 특정 MFC 클래스를 사용하면 프레임워크에 이 클래스가 필요합니다.
...
|[CMDIChildWndEx:: CanShowOnWindowsList](#canshowonwindowslist)|[Cmfcwindowsmanagerdialog 클래스](../../mfc/reference/cmfcwindowsmanagerdialog-class.md) 대화 상자에 MDI 자식 창 이름을 표시할 수 있으면 TRUE를 반환 합니다. 그렇지 않으면 FALSE를 반환합니다.|
|`CMDIChildWndEx::CreateObject`|이 클래스 형식의 동적 인스턴스를 만들기 위해 프레임 워크에서 호출 됩니다.|
|[CMDIChildWndEx::D ockPane](#dockpane)|창을 도킹 합니다.|
|[CMDIChildWndEx::D ockPaneLeftOf](#dockpaneleftof)|창을 다른 창의 왼쪽에 도킹합니다.|
...
|[CMDIChildWndEx:: Get프레임 아이콘](#getframeicon)|MDI 자식 창 아이콘을 검색 하기 위해 프레임 워크에서 호출 됩니다.|
...
|[CMDIChildWndEx:: IsReadOnly](#isreadonly)|자식 창에 표시 된 문서가 읽기 전용 이면 TRUE를 반환 합니다. 그렇지 않으면 FALSE를 반환합니다.|
...
|[CMDIChildWndEx:: IsTabbedPane](#istabbedpane)|MDI 자식 창에 도킹 창이 포함 되어 있으면 TRUE를 반환 합니다. 그렇지 않으면 FALSE를 반환합니다.|
|[CMDIChildWndEx:: Istask바 Tabssupportenabled](#istaskbartabssupportenabled)|MDI 자식이 Windows 7 작업 표시줄 탭에 표시 될 수 있는지 여부를 나타냅니다.|
...
|[CMDIChildWndEx:: m_dwDefaultTaskbarTabPropertyFlags](#m_dwdefaulttaskbartabpropertyflags)|탭 (MDI 자식)이 Windows 7 작업 표시줄 탭에 등록 될 때 프레임 워크에서 Settask Tabproperties 메서드로 전달 되는 플래그의 조합입니다. 기본 조합은 STPF_USEAPPPEEKWHENACTIVE &#124; STPF_USEAPPTHUMBNAILWHENACTIVE 됩니다.|
...
|[CMDIChildWndEx::P aneFromPoint](#panefrompoint)|지정 된 지점을 포함 하는 창을 반환 합니다.|
...
|[CMDIChildWndEx:: Settask바 Tabactive](#settaskbartabactive)|해당 하는 Windows 7 작업 표시줄 탭을 활성화 합니다.|
|[CMDIChildWndEx:: Settaskorder Taborder](#settaskbartaborder)|Windows 7 작업 표시줄 탭의 지정 된 창 앞에 MDI 자식을 삽입 합니다.|
...
|[CMDIChildWndEx:: Unregistertask바코드 탭](#unregistertaskbartab)|Windows 7 작업 표시줄 탭에서 MDI 자식을 제거 합니다.|
|[CMDIChildWndEx:: Updatetask바 Tabicon](#updatetaskbartabicon)|Windows 7 작업 표시줄 탭 아이콘을 업데이트 합니다.|

## <a name="remarks"></a>설명

MDI 응용 프로그램에서 확장 된 도킹 기능을 활용 하려면 CMDIChildWnd 대신 응용 프로그램의 MDI 자식 창 클래스를 파생 시킵니다 `CMDIChildWndEx` . [](../../mfc/reference/cmdichildwnd-class.md)

## <a name="example"></a>예제

다음 예제에서는에서 클래스를 파생 시킵니다 `CMDIChildWndEx` . 이 코드 조각은 [VisualStudioDemo 샘플: MFC Visual Studio 응용 프로그램](../../overview/visual-cpp-samples.md)에서 제공 됩니다.
...
**창이 창 대화 상자** 에 표시 될 수 있으면 TRUE입니다. 그렇지 않으면 FALSE입니다.

### <a name="remarks"></a>설명

파생 클래스에서이 메서드를 재정의 하 고 창이 **Windows** 대화 상자에 표시 되지 않아야 하는 경우 FALSE를 반환 합니다. 이 함수는에서 호출 됩니다 `CMFCWindowsManagerDialog` .
...
문서는 MDI 자식 창에 표시 되는 내용입니다. 일반적으로 창에는 파일에서 로드 되거나 파일에 저장 된 데이터가 표시 됩니다. 따라서 문서 이름은 파일의 이름입니다. 의 기본 구현은 `GetDocumentName` 에서 가져온 문자열을 반환 합니다 `CDocument::GetPathName` .
...
다음 예제에서는 `GetDocumentName` 메서드를 사용하는 방법을 보여 줍니다. 이 코드 조각은 [VisualStudioDemo 샘플: MFC Visual Studio 응용 프로그램](../../overview/visual-cpp-samples.md)에서 제공 됩니다.

[!code-cpp[NVC_MFC_VisualStudioDemo#17](../../mfc/codesnippet/cpp/cmdichildwndex-class_2.cpp)]

## <a name="cmdichildwndexgetframeicon"></a><a name="getframeicon"></a> CMDIChildWndEx:: Get프레임 아이콘
...
기본적으로이 메서드는 창 아이콘을 반환 합니다. `GetFrameIcon`파생 클래스에서를 재정의 `CMDIChildWndEx` 하 여이 동작을 사용자 지정 합니다.
...
기본적으로이 메서드는 창 텍스트를 반환 합니다. `GetFrameText`파생 클래스에서를 재정의 `CMDIChildWndEx` 하 여이 동작을 사용자 지정 합니다.
...
다음 예제에서는 메서드를 재정의 하는 방법을 보여 줍니다 `IsReadOnly` . 이 코드 조각은 [VisualStudioDemo 샘플: MFC Visual Studio 응용 프로그램](../../overview/visual-cpp-samples.md)에서 제공 됩니다.
...
`CBasePane`지정 된 지점을 포함 하는 파생 개체에 대 한 포인터 이거나 창이 없는 경우 NULL입니다.
...
## <a name="cmdichildwndexupdatetaskbartabicon"></a><a name="updatetaskbartabicon"></a> CMDIChildWndEx:: Updatetask바 Tabicon
...
## <a name="cmdichildwndexunregistertaskbartab"></a><a name="unregistertaskbartab"></a> CMDIChildWndEx:: Unregistertask바코드 탭
...
## <a name="cmdichildwndexsettaskbartabproperties"></a><a name="settaskbartabproperties"></a> CMDIChildWndEx:: Settask바 Tabproperties
...
## <a name="cmdichildwndexsettaskbartaborder"></a><a name="settaskbartaborder"></a> CMDIChildWndEx:: Settaskorder Taborder
...
*pWndBefore*<br/>
축소판 그림이 왼쪽에 삽입 되는 MDI 자식 창에 대 한 포인터입니다. 이 창은를 통해 이미 등록 되어 있어야 `RegisterTaskbarTab` 합니다. 이 값이 NULL 이면 새 미리 보기가 목록의 끝에 추가 됩니다.

### <a name="remarks"></a>설명

## <a name="cmdichildwndexsettaskbartabactive"></a><a name="settaskbartabactive"></a> CMDIChildWndEx:: Settask바 Tabactive
...
파생 클래스에서이 메서드를 재정의 하 고 MDI 자식의 실시간 미리 보기에 대해 유효한 32bpp 비트맵을 반환 합니다. 이 메서드는 MDI 자식이 Windows 7 작업 표시줄 탭에 표시 되는 경우에만 호출 됩니다. NULL을 반환 하는 경우 MFC는 기본 처리기를 호출 하 고 또는을 사용 하 여 비트맵을 가져옵니다 `PrintClient` `PrintWindow` .
...
`SetTaskbarTabProperties`탭 (MDI 자식)이 Windows 7 작업 표시줄 탭에 등록 될 때 프레임 워크에서 메서드로 전달 되는 플래그의 조합입니다.
...
기본 조합은 STPF_USEAPPPEEKWHENACTIVE &#124; STPF_USEAPPTHUMBNAILWHENACTIVE 됩니다.
...
## <a name="cmdichildwndexistaskbartabssupportenabled"></a><a name="istaskbartabssupportenabled"></a> CMDIChildWndEx:: Istask바 Tabssupportenabled
...
는 개체에 대 한 유효한 포인터를 반환 해야 합니다 `CWnd` .이 개체는 해당 미리 보기가이 MDI 자식과 관련 된 Windows 7 작업 표시줄 탭에 표시 되어야 합니다. 기본 구현에서는 AFX_IDW_PANE_FIRST 컨트롤 ID (일반적으로 파생 클래스)를 사용 하 여이 MDI 자식의 자식 창을 반환 합니다 `CView` .
...
`CMDITabProxyWnd`Windows 7 작업 표시줄 탭에 등록 된 개체에 대 한 포인터입니다.
