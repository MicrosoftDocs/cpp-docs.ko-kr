---
description: '자세히 알아보기: COleClientItem 클래스'
...
|[COleClientItem:::](#activateas)|항목을 다른 형식으로 활성화 합니다.|
...
|[COleClientItem:: Convertto-html](#convertto)|항목을 다른 형식으로 변환 합니다.|
...
|[COleClientItem::D oDragDrop](#dodragdrop)|끌어서 놓기 작업을 수행 합니다.|
...
|[COleClientItem::GetClipboardData](#getclipboarddata)|멤버 함수를 호출 하 여 클립보드에 배치할 데이터를 가져옵니다 `CopyToClipboard` .|
|[COleClientItem:: GetDocument](#getdocument)|`COleDocument`현재 항목을 포함 하는 개체를 반환 합니다.|
...
|[COleClientItem:: GetIconicMetafile 파일](#geticonicmetafile)|항목의 아이콘을 그리는 데 사용 되는 메타 파일을 가져옵니다.|
...
|[COleClientItem:: Reload](#reload)|를 호출한 후 항목을 다시 로드 `ActivateAs` 합니다.|
...
|[COleClientItem:: SetIconicMetafile 파일](#seticonicmetafile)|항목의 아이콘을 그리는 데 사용 되는 메타 파일을 캐시 합니다.|
...
`COleClientItem`[Coledocument](../../mfc/reference/coledocument-class.md), [COleLinkingDoc](../../mfc/reference/colelinkingdoc-class.md)또는 [COleServerDoc](../../mfc/reference/coleserverdoc-class.md) 클래스와 함께 사용할 수 있습니다. 를 사용 하려면 `COleClientItem` 클래스에서 클래스를 파생 시키고, 컨테이너가 항목의 변경 내용에 응답 하는 방식을 정의 하는 [OnChange](#onchange) 멤버 함수를 구현 합니다. 내부 활성화를 지원 하려면 [Ongetitemposition](#ongetitemposition) 멤버 함수를 재정의 합니다. 이 함수는 OLE 항목의 표시 된 위치에 대 한 정보를 제공 합니다.

컨테이너 인터페이스를 사용 하는 방법에 대 한 자세한 내용은 컨테이너 [: 컨테이너 구현](../../mfc/containers-implementing-a-container.md) 및 [활성화](../../mfc/activation-cpp.md)문서를 참조 하세요.

> [!NOTE]
> Windows SDK는 포함 된 항목과 연결 된 항목을 "objects"로 참조 하 고 항목 형식을 "클래스"로 참조 합니다. 이 참조는 "item" 이라는 용어를 사용 하 여 OLE 엔터티와 해당 c + + 개체를 구분 하 고 "type" 이라는 용어를 사용 하 여 c + + 클래스와 OLE 범주를 구분 합니다.
...
서버 응용 프로그램이 MFC 라이브러리를 사용 하 여 작성 된 경우이 함수는 해당 개체의 [Ondoverb](../../mfc/reference/coleserveritem-class.md#ondoverb) 멤버 함수가 `COleServerItem` 실행 되도록 합니다.
...
## <a name="coleclientitemactivateas"></a><a name="activateas"></a> COleClientItem:::
...
*rDataObject*<br/>
`COleDataObject`OLE 항목의 데이터에 대 한 액세스를 허용 하도록 초기화 될 개체에 대 한 참조입니다.
...
컨테이너 응용 프로그램이 지정 된 개체에서 포함 된 개체를 만들 수 있는지 여부를 확인 `COleDataObject` 합니다.
...
컨테이너가 개체에서 포함 된 개체를 만들 수 있으면 0이 아니고 `COleDataObject` , 그렇지 않으면 0입니다.
...
컨테이너 응용 프로그램이 지정 된 개체에서 연결 된 개체를 만들 수 있는지 여부를 확인 `COleDataObject` 합니다.
...
컨테이너가 개체에서 연결 된 개체를 만들 수 있는 경우 0이 아닙니다 `COleDataObject` .
...
*dwCloseOption*<br/>
OLE 항목이 로드 된 상태로 반환 될 때 저장 되는 상황을 지정 하는 플래그입니다. 다음 값 중 하나일 수 있습니다.
...
개체를 생성 `COleClientItem` 하 여 컨테이너 문서의 문서 항목 컬렉션에 추가 합니다 .이 개체는 c + + 개체만 생성 하 고 OLE 초기화는 수행 하지 않습니다.
...
*lpFormatEtc*<br/>
*렌더링이* OLERENDER_FORMAT 또는 OLERENDER_DRAW 경우 사용 되는 [FORMATETC](/windows/win32/api/objidl/ns-objidl-formatetc) 구조체에 대 한 포인터입니다. *Cfformat* 으로 지정 된 클립보드 형식을 벗어나는 추가 형식 정보를 지정 하려는 경우에만이 매개 변수에 대 한 값을 제공 합니다. 이 매개 변수를 생략 하면 구조체의 다른 필드에 기본값이 사용 됩니다 `FORMATETC` .
...
개체에서 포함 된 항목을 만들려면이 함수를 호출 `COleDataObject` 합니다.
...
데이터 전송 작업 (예: 클립보드 또는 끌어서 놓기 작업에서 붙여넣기)은 `COleDataObject` 서버 응용 프로그램에서 제공 하는 정보를 포함 하는 개체를 제공 합니다. 일반적으로 [CView:: OnDrop](../../mfc/reference/cview-class.md#ondrop)의 재정의에 사용 됩니다.
...
개체에서 연결 된 항목을 만들려면이 함수를 호출 `COleDataObject` 합니다.
...
사용자가 링크를 만들어야 함을 나타내는 경우 drop 작업 중에이를 호출 합니다. 이를 사용 하 여 붙여넣기 편집 명령을 처리할 수도 있습니다. 이 메서드는의 프레임 워크 `COleClientItem::CreateLinkFromClipboard` 및 Link 옵션을 선택한 경우 [COlePasteSpecialDialog:: CreateItem](../../mfc/reference/colepastespecialdialog-class.md#createitem) 에서 호출 됩니다.
...
자세한 내용은 Windows SDK [Olecreate](/windows/win32/api/ole/nf-ole-olecreate), [OLERENDER](/windows/win32/api/oleidl/ne-oleidl-olerender)및 [](/windows/win32/api/objidl/ns-objidl-formatetc) 를 참조 하세요.
...
개체에서 정적 항목을 만들려면이 함수를 호출 `COleDataObject` 합니다.
...
정적 항목에는 프레젠테이션 데이터는 포함 되지만 네이티브 데이터는 포함 되지 않습니다. 따라서 편집할 수 없습니다. 이는 기본적으로 [CreateStaticFromClipboard](#createstaticfromclipboard) 와 동일 합니다. 단, 정적 항목은 클립보드에서 뿐만 아니라 임의로 만들 수 있습니다 `COleDataObject` .
...
응용 프로그램에서 실행 취소를 지 원하는 경우에는를 호출 하지 말고 `Deactivate` [Deactivateui](#deactivateui)를 호출 합니다.
...
이 함수는 [릴리스](#release) 멤버 함수를 호출 하며,이 함수는 항목에 대 한 c + + 개체를 삭제 하 여 문서에서 OLE 항목을 영구적으로 제거 합니다. OLE 항목이 포함 되어 있으면 항목에 대 한 네이티브 데이터가 삭제 됩니다. 항상 실행 중인 서버를 닫습니다. 따라서 항목이 열린 링크인 경우이 함수는이를 닫습니다.
...
## <a name="coleclientitemdoverb"></a><a name="doverb"></a> COleClientItem::D 초과 b

`DoVerb`을 호출 하 여 지정 된 동사를 실행 합니다.
...
## <a name="coleclientitemdraw"></a><a name="draw"></a> COleClientItem: 원시:D
...
*lpBounds*<br/>
OLE 항목을 그릴 경계 사각형을 정의 하는 [Crect](../../atl-mfc-shared/reference/crect-class.md) 개체 또는 구조에 대 한 포인터 `RECT` 입니다 (장치 컨텍스트에 따라 결정 되는 논리 단위).
...
함수는의 [OnDraw](../../mfc/reference/coleserveritem-class.md#ondraw) 멤버 함수에서 만든 OLE 항목의 메타 파일 표현을 사용할 수 있습니다 `COleServerItem` .
...
*lpSize*<br/>
`SIZE`크기 정보를 수신 하는 구조체 또는 [csize](../../atl-mfc-shared/reference/csize-class.md) 개체에 대 한 포인터입니다.
...
이 함수는 [Getextent](#getextent)와 동일한 정보를 제공 합니다. 그러나를 호출 하 여 `GetCachedExtent` [OnChange](#onchange)와 같은 다른 OLE 처리기를 처리 하는 동안 익스텐트 정보를 가져올 수 있습니다. 차원이 MM_HIMETRIC 단위입니다.

이는 `GetCachedExtent` [IOleObject](/windows/win32/api/oleidl/nn-oleidl-ioleobject) 인터페이스를 사용 하 여이 항목의 범위를 가져오는 대신 [IViewObject2](/windows/win32/api/oleidl/nn-oleidl-iviewobject2) 인터페이스를 사용 하기 때문에 가능 합니다. `IViewObject2`COM 개체는 [Iviewobject](/windows/win32/api/oleidl/nf-oleidl-iviewobject-draw)에 대 한 이전 호출에 사용 된 익스텐트 정보를 캐시 합니다.:D raw.
...
`GetClipboardData` 는 [OnGetClipboardData](#ongetclipboarddata)의 기본 구현으로 호출 됩니다. `OnGetClipboardData`에서 제공 하는 것 외에 데이터 형식을 제공 하려는 경우에만를 재정의 `CopyToClipboard` 합니다. `COleDataSource`를 호출 하기 전이나 후에 해당 형식을 개체에 배치한 `CopyToClipboard` 다음 `COleDataSource` 개체를 [Coledatasource:: setclipboard](../../mfc/reference/coledatasource-class.md#setclipboard) 함수로 전달 합니다. 예를 들어 컨테이너 문서에서 OLE 항목의 위치를 클립보드에 배치 하려는 경우 해당 정보를 전달 하 고를 호출 하기 전에에 배치 하기 위해 고유한 형식을 정의 해야 `COleDataSource` `CopyToClipboard` 합니다.
...
이 포인터 `COleDocument` 를 사용 하면 생성자에 인수로 전달한 개체에 액세스할 수 있습니다 `COleClientItem` .
...
*lpSize*<br/>
크기 정보를 수신 하는 구조체 또는 개체에 대 한 포인터 `SIZE` `CSize` 입니다.
...
MFC 라이브러리를 사용 하 여 서버 응용 프로그램을 작성 한 경우이 함수를 사용 하면 해당 개체의 [Ongetextent](../../mfc/reference/coleserveritem-class.md#ongetextent) 멤버 함수가 `COleServerItem` 호출 됩니다. 검색 된 크기는 [Setextent](#setextent) 멤버 함수에 의해 마지막으로 설정 된 크기와 다를 수 있습니다. 로 지정 된 크기는 `SetExtent` 제안으로 처리 됩니다. 차원이 MM_HIMETRIC 단위입니다.
...
## <a name="coleclientitemgeticonicmetafile"></a><a name="geticonicmetafile"></a> COleClientItem:: GetIconicMetafile 파일
...
열거형 값으로,,,, 중 `COleClientItem::ItemState` 하나일 수 있습니다. `emptyState` `loadedState` `openState` `activeState` `activeUIState` 이러한 상태에 대 한 자세한 내용은 [컨테이너: Client-Item 상태](../../mfc/containers-client-item-states.md)문서를 참조 하세요.

### <a name="remarks"></a>설명

OLE 항목의 상태가 변경 될 때 알리도록 하려면 [OnChange](#onchange) 멤버 함수를 사용 합니다.

자세한 내용은 [컨테이너: Client-Item 상태](../../mfc/containers-client-item-states.md)문서를 참조 하세요.
...
부울 값 FALSE를 반환 하는 멤버 함수 또는 NULL을 반환 하는 다른 멤버 함수에 대해 `GetLastStatus` 더 자세한 오류 정보를 반환 합니다. 대부분의 OLE 멤버 함수는 심각한 오류에 대 한 예외를 throw 합니다. 을 해석 하는 방법에 대 한 구체적인 정보는 마지막으로 값이 반환 된 기본 OLE 호출에 따라 다릅니다.
...
항목을 현재 편집 하 고 있는지 여부에 따라 다른 논리를 실행 하는 것이 일반적입니다. 함수는 현재 항목 상태가 또는와 같은지 여부를 확인 합니다 `activeState` `activeUIState` .
...
기본 구현에서는 나중에 호출할 때 서버의 HWND를 기억 합니다 `GetServerWindow` .
...
*nCode*<br/>
서버에서이 항목을 변경한 이유입니다. 다음 값 중 하나일 수 있습니다.
...
*dwParam*<br/>
*Ncode* 가 OLE_SAVED 또는 OLE_CLOSED 이면이 매개 변수는 사용 되지 않습니다. *Ncode* 가 OLE_CHANGED 이면이 매개 변수는 변경 된 OLE 항목의 모양을 지정 합니다. 가능한 값은 [COleClientItem::D raw](#draw)의 *dwparam* 매개 변수를 참조 하세요. *Ncode* 가 OLE_CHANGED_STATE 되는 경우이 매개 변수는 `COleClientItem::ItemState` 열거 값 이며 입력 되는 상태를 설명 합니다. `emptyState`,, `loadedState` `openState` , `activeState` 또는 값 중 하나를 사용할 수 있습니다 `activeUIState` .

### <a name="remarks"></a>설명

서버 응용 프로그램이 MFC 라이브러리를 사용 하 여 작성 되는 경우이 함수는 `Notify` 또는의 멤버 함수에 대 한 응답으로 호출 됩니다 `COleServerDoc` `COleServerItem` . 기본 구현은 *Ncode* 가 OLE_CHANGED 또는 OLE_SAVED 경우 컨테이너 문서를 수정 된 것으로 표시 합니다.
...
내부 활성화 상태 ()에서 로드 된 상태로 전환 될 때 프레임 워크에서 호출 `activeState` 됩니다. 즉, 내부 활성화 후에 비활성화 됩니다.
...
`COleDataSource` [CopyToClipboard](#copytoclipboard) 또는 [dodragdrop](#dodragdrop) 멤버 함수를 호출 하 여 클립보드에 배치 되는 모든 데이터를 포함 하는 개체를 가져오기 위해 프레임 워크에서 호출 됩니다.
...
*lpMenuWidths*<br/>
는 파일, 편집, 컨테이너, 개체, 창, 도움말 메뉴 그룹 각각에 있는 메뉴의 수를 나타내는 6 개의 LONG 값 배열을 가리킵니다. 컨테이너 응용 프로그램은이 배열의 요소 0, 2 및 4에 해당 하는 파일, 컨테이너 및 창 메뉴 그룹을 담당 합니다.
...
서버에서를 반복적으로 호출 하는 경우 *pMenuShared* 의 하위 메뉴를 둘 이상의 복합 메뉴에서 공유할 수 있습니다 `OnInsertMenus` . 따라서 재정의에서 하위 메뉴를 삭제 해서는 안 됩니다. 이러한 하위 메뉴는 분리 해야 합니다 `OnRemoveMenus` .
...
*pMenuShared*<br/>
[Oninsertmenus](#oninsertmenus) 멤버 함수 및 함수에 대 한 호출로 생성 된 복합 메뉴에 대 한 포인터 `InsertMenu` 입니다.

*holemenu*<br/>
함수에서 반환 된 메뉴 설명자에 대 한 핸들 `OleCreateMenuDescriptor` 또는 디스패치 코드를 제거할 경우 NULL입니다.
...
기본 구현에서는 프레임 창 제목이 변경 되지 않습니다. 응용 프로그램에 다른 프레임 제목을 원하는 경우이 함수를 재정의 합니다 (예 *: "*  -   Microsoft Excel-REPORT.DOC 의 스프레드시트"). 이는 고급 재정의 가능입니다.
...
`Release` 소멸자에 의해 호출 됩니다 `COleClientItem` .
...
`Reload`지 수를 호출 하 여 다른 형식의 항목으로 항목을 활성화 한 후에 함수를 [호출 합니다](#activateas).
...
`Run`항목을 활성화 하기 전에 멤버 함수를 호출 하 여 서버 응용 프로그램을 시작 합니다. 이 작업은 [활성화](#activate) 및 [doverb](#doverb)에 의해 자동으로 수행 되므로 일반적으로이 함수를 호출할 필요가 없습니다. [Doverb](#doverb)를 실행 하기 전에 [setextent](#setextent)같은 항목 특성을 설정 하기 위해 서버를 실행 해야 하는 경우이 함수를 호출 합니다.
...
MFC 라이브러리를 사용 하 여 서버 응용 프로그램을 작성 한 경우에는 해당 개체의 [Onsetextent](../../mfc/reference/coleserveritem-class.md#onsetextent) 멤버 함수가 `COleServerItem` 호출 됩니다. 그러면 OLE 항목의 표시를 적절 하 게 조정할 수 있습니다. 차원은 MM_HIMETRIC 단위 여야 합니다. 사용자가 OLE 항목의 크기를 조정 하거나 일부 형식의 레이아웃 협상을 지 원하는 경우이 함수를 호출 합니다.
...
## <a name="coleclientitemseticonicmetafile"></a><a name="seticonicmetafile"></a> COleClientItem:: SetIconicMetafile 파일
...
이 함수에 대 한 인수는 OLE 시스템에서 대상 장치를 식별 하는 데 사용 하는 정보를 포함 합니다. 구조에는 `PRINTDLG` Windows에서 일반 인쇄 대화 상자를 초기화 하는 데 사용 하는 정보가 포함 되어 있습니다. 사용자가 대화 상자를 닫은 후에는이 구조에서 사용자의 선택 항목에 대 한 정보를 반환 합니다. `m_pd` [CPrintDialog](../../mfc/reference/cprintdialog-class.md) 개체의 멤버는 `PRINTDLG` 구조체입니다.
