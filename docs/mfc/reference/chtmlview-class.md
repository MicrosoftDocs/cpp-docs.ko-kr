---
description: '자세히 알아보기: CHtmlView 클래스'
title: CHtmlView 클래스
ms.date: 11/04/2016
f1_keywords:
- CHtmlView
...
|[CHtmlView:: Exec양식 명령](#execformscommand)|`IOleCommandTarget::Exec` 메서드를 사용하여 지정된 명령을 실행합니다.|
...
|[CHtmlView:: GetFullScreen 화면](#getfullscreen)|WebBrowser 컨트롤이 전체 화면 모드 또는 표준 창 모드로 작동 중인지를 나타냅니다.|
|[CHtmlView:: Myfont.getheight](#getheight)|Internet Explorer 주 창의 높이를 검색합니다.|
...
|[CHtmlView:: On프레임 창 활성화](#onframewindowactivate)|컨테이너의 최상위 프레임 창이 활성화 또는 비활성화될 경우 개체에 알리기 위해 [IOleInPlaceActiveObject::OnFrameWindowActivate](/windows/win32/api/oleidl/nf-oleidl-ioleinplaceactiveobject-onframewindowactivate) 에서 호출됩니다.|
|[CHtmlView:: OnFullScreen 화면](#onfullscreen)|FullScreen 속성이 변경되면 호출됩니다.|
...
|[CHtmlView:: OnPropertyChange](#onpropertychange)|[PutProperty](#putproperty) 메서드가 속성 값을 변경했음을 애플리케이션에 알리기 위해 호출됩니다.|
...
|[CHtmlView::P utProperty](#putproperty)|지정된 개체와 연결된 속성의 값을 설정합니다.|
|[CHtmlView:: Query Command](#queryformscommand)|사용자 인터페이스 이벤트에 의해 생성되는 하나 이상 명령의 상태를 쿼리합니다.|
...
|[CHtmlView:: SetFullScreen 화면](#setfullscreen)|컨트롤이 전체 화면 모드 또는 표준 창 모드로 작동 중인지를 결정할 값을 설정합니다. WebBrowser 컨트롤은 무시됩니다. Internet Explorer에만 해당합니다.|
...
이 멤버 함수를 호출 하 여 Internet Explorer 실행 파일에 대 한 WebBrowser 컨트롤 또는 컨테이너를 만듭니다.
...
*lpszClassName*<br/>
Windows 클래스의 이름을 나타내는 null로 끝나는 문자열을 가리킵니다. 클래스 이름은 [AfxRegisterWndClass](../../mfc/reference/application-information-and-management.md#afxregisterwndclass) global 함수 또는 Windows 함수에 등록 된 모든 이름일 수 있습니다 `RegisterClass` . NULL 인 경우 미리 정의 된 기본 [CFrameWnd](../../mfc/reference/cframewnd-class.md) 특성을 사용 합니다.
...
*pContainer*<br/>
컨트롤을 포함 하는 [COleControlContainer](../../mfc/reference/colecontrolcontainer-class.md) 개체에 대 한 포인터입니다.

*ppSite*<br/>
컨트롤에 대 한 사이트를 제공 하는 [COleControlSite](../../mfc/reference/colecontrolsite-class.md) 개체에 대 한 포인터에 대 한 포인터입니다.
...
## <a name="chtmlviewexecformscommand"></a><a name="execformscommand"></a> CHtmlView:: Exec양식 명령
...
*pVarIn*<br/>
입력 인수를 포함 하는 구조체에 대 한 포인터 `VARIANT` 입니다. NULL일 수 있습니다.

*pVarOut*<br/>
명령 출력을 수신 하는 구조체에 대 한 포인터 `VARIANT` 입니다. NULL일 수 있습니다.
...
*cmdexecopt*<br/>
명령 실행에 대해 설정 된 옵션입니다.

*pvaIn*<br/>
명령 입력 인수를 지정 하는 데 사용 되는 variant입니다.
...
Windows SDK에서 [IWebBrowser2:: ExecWB](/previous-versions//aa752117\(v=vs.85\)) 를 참조 하세요.
...
주소 표시줄이 표시 되 면 0이 아닌 값입니다. 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

Internet Explorer에 적용 됩니다. WebBrowser 컨트롤과 함께이 호출을 사용 하는 경우 오류를 반환 하지 않지만이 호출을 무시 합니다.
...
`IDispatch`활성 문서 개체의 인터페이스에 대 한 포인터입니다. 자세한 내용은 [Implementing the IDispatch Interface](/previous-versions/windows/desktop/automat/implementing-the-idispatch-interface)을 참조하십시오.
...
이 멤버 함수를 호출 하 여 WebBrowser 컨트롤이 탐색 또는 다운로드 작업에 참여 하는지 여부를 확인 합니다.
...
웹 브라우저가 사용 중인 경우에는 0이 아닙니다. 그렇지 않으면 0입니다.
...
이 멤버 함수를 호출 하 여 웹 브라우저의 컨테이너로 계산 되는 개체를 검색 합니다.
...
`IDispatch`활성 문서 개체의 인터페이스에 대 한 포인터입니다.
...
Internet Explorer에서 현재 표시 하는 파일의 전체 경로를 검색 하려면이 멤버 함수를 호출 합니다.
...
현재 표시 된 파일의 경로와 이름을 포함 하는 [CString](../../atl-mfc-shared/reference/cstringt-class.md) 개체입니다. 경로 및 파일 이름이 없으면 빈을 `GetFullName` 반환 `CString` 합니다.

### <a name="remarks"></a>설명

Internet Explorer에 적용 됩니다. WebBrowser 컨트롤과 함께이 호출을 사용 하는 경우 오류를 반환 하지 않지만이 호출을 무시 합니다.

## <a name="chtmlviewgetfullscreen"></a><a name="getfullscreen"></a> CHtmlView:: GetFullScreen 화면

이 멤버 함수를 호출 하 여 WebBrowser 컨트롤이 전체 화면 모드 또는 표준 창 모드에서 작동 하는지 여부를 확인 합니다.
...
WebBrowser가 전체 화면 모드에서 작동 하는 경우 0이 아닙니다. 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

전체 화면 모드에서는 Internet Explorer 주 창이 최대화 되 고 상태 표시줄, 도구 모음, 메뉴 모음 및 제목 표시줄이 숨겨집니다.

Internet Explorer 및 WebBrowser에 적용됩니다.

## <a name="chtmlviewgetheight"></a><a name="getheight"></a> CHtmlView:: Myfont.getheight

이 멤버 함수를 호출 하 여 WebBrowser 컨트롤 프레임 창의 높이 (픽셀 단위)를 검색 합니다.
...
이 멤버 함수를 호출 하 여 활성 문서에 대 한 HTML 문서를 검색 합니다.
...
WebBrowser 컨트롤의 왼쪽 내부 가장자리와 해당 컨테이너의 왼쪽 가장자리 사이의 거리를 검색 하려면이 멤버 함수를 호출 합니다.
...
이 멤버 함수를 호출 하 여 WebBrowser에 표시 되는 리소스의 이름을 가져옵니다.
...
WebBrowser에 현재 표시 된 리소스의 이름을 포함 하는 [CString](../../atl-mfc-shared/reference/cstringt-class.md) 개체입니다.

### <a name="remarks"></a>설명

리소스가 World Wide Web의 HTML 페이지인 경우에는 이름이 해당 페이지의 제목입니다. 리소스가 네트워크 또는 로컬 컴퓨터의 폴더 또는 파일 인 경우이 이름은 폴더 또는 파일의 UNC 또는 전체 경로입니다.

Internet Explorer 및 WebBrowser에 적용됩니다.

## <a name="chtmlviewgetlocationurl"></a><a name="getlocationurl"></a> CHtmlView:: GetLocationURL

이 멤버 함수를 호출 하 여 WebBrowser 컨트롤이 현재 표시 하 고 있는 리소스의 URL을 검색 합니다.
...
WebBrowser에 현재 표시 된 리소스의 URL을 포함 하는 [CString](../../atl-mfc-shared/reference/cstringt-class.md) 개체입니다.

### <a name="remarks"></a>설명

리소스가 네트워크 또는 로컬 컴퓨터의 폴더 또는 파일 인 경우이 이름은 폴더 또는 파일의 UNC 또는 전체 경로입니다.

Internet Explorer 및 WebBrowser에 적용됩니다.

## <a name="chtmlviewgetmenubar"></a><a name="getmenubar"></a> CHtmlView:: GetMenuBar

메뉴 모음을 표시할지 여부를 확인 하려면이 멤버 함수를 호출 합니다.
...
메뉴 모음이 표시 되 면 0이 아닌 값입니다. 그렇지 않으면 0입니다.
...
웹 브라우저가 오프 라인으로 작동 하는지 여부를 확인 하려면이 멤버 함수를 호출 합니다.
...
웹 브라우저가 현재 오프 라인 상태인 경우 0이 아닙니다. 그렇지 않으면 0입니다.
...
이 멤버 함수를 호출 하 여 WebBrowser 컨트롤의 부모 개체에 대 한 포인터를 검색 합니다.
...
`IDispatch`WebBrowser 컨트롤의 부모인 개체의 인터페이스에 대 한 포인터입니다.
...
이 멤버 함수를 호출 하 여 현재 컨트롤과 연결 된 속성의 값을 가져옵니다.
...
첫 번째 버전에서 성공적으로 완료 되 면 0이 아닌 값을 입력 합니다. 그렇지 않으면 0입니다. 두 번째 버전에서 [COleVariant](../../mfc/reference/colevariant-class.md) 개체입니다.
...
이 멤버 함수를 호출 하 여 WebBrowser 개체의 준비 상태를 검색 합니다.
...
Windows SDK 설명 된 [READYSTATE](/previous-versions//aa768362\(v=vs.85\)) 값입니다.
...
이 멤버 함수를 호출 하 여 WebBrowser 개체가 대상 이름 확인을 위한 최상위 브라우저로 등록 되었는지 여부를 확인 합니다.
...
브라우저가 최상위 브라우저로 등록 된 경우 0이 아닌 값입니다. 그렇지 않으면 0입니다.
...
이 멤버 함수를 호출 하 여 WebBrowser 컨트롤이 탐색을 위한 놓기 대상으로 등록 되었는지 여부를 확인 합니다.
...
브라우저가 놓기 대상으로 등록 된 경우 0이 아닙니다. 그렇지 않으면 0입니다.
...
이 멤버 함수를 호출 하 여 WebBrowser 컨트롤에 대화 상자를 표시할 수 있는지 여부를 확인 합니다.
...
WebBrowser 컨트롤에서 대화 상자를 표시할 수 없는 경우 0이 아닌 값입니다. 그렇지 않으면 0입니다.
...
*refString*<br/>
소스 코드를 포함 하는 [CString](../../atl-mfc-shared/reference/cstringt-class.md) 입니다.

### <a name="remarks"></a>설명

이 함수는 소스 코드가에서 반환 된다는 점을 제외 하 고 Internet Explorer의 "소스 보기" 명령과 동일 합니다 `CString` .

## <a name="chtmlviewgetstatusbar"></a><a name="getstatusbar"></a> CHtmlView:: GetStatusBar

WebBrowser 컨트롤에서 상태 표시줄을 표시할지 여부를 결정 하려면이 멤버 함수를 호출 합니다.
...
상태 표시줄을 표시할 수 있는 경우 0이 아닙니다. 그렇지 않으면 0입니다.
...
웹 브라우저가 극장 모드 인지 여부를 확인 하려면이 멤버 함수를 호출 합니다.
...
웹 브라우저가 극장 모드 이면 0이 아닌 것입니다. 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

웹 브라우저가 극장 모드에 있으면 브라우저 주 창이 전체 화면을 채우고, 최소 탐색 도구 집합이 있는 도구 모음이 나타나고, 상태 표시줄이 화면의 오른쪽 위에 표시 됩니다.

Internet Explorer 및 WebBrowser에 적용됩니다.

## <a name="chtmlviewgettoolbar"></a><a name="gettoolbar"></a> CHtmlView:: GetToolBar

이 멤버 함수를 호출 하 여 도구 모음이 표시 되는지 여부를 확인 합니다.
...
도구 모음을 표시할지 여부를 나타내는 값입니다. 도구 모음이 표시 되는 경우 0이 아님 그렇지 않으면 0입니다.

## <a name="chtmlviewgettop"></a><a name="gettop"></a> CHtmlView:: GetTop

이 멤버 함수를 호출 하 여 WebBrowser 컨트롤의 주 창에서 위쪽 가장자리의 화면 좌표를 검색 합니다.
...
Internet Explorer가 WebBrowser 컨트롤의 최상위 컨테이너 인지 여부를 확인 하려면이 멤버 함수를 호출 합니다.
...
이 멤버 함수를 호출 하 여 포함 된 활성 문서의 형식 이름을 검색 합니다.
...
포함 된 활성 문서의 형식 이름을 포함 하는 [CString](../../atl-mfc-shared/reference/cstringt-class.md) 개체입니다.
...
이 멤버 함수를 호출 하 여 포함 된 개체가 표시 되는지 여부를 확인 합니다.
...
개체가 표시 되는 경우 0이 아닙니다. 그렇지 않으면 0입니다.
...
Internet Explorer 인터넷 옵션 대화 상자 또는 제어판에서 액세스할 수 있는 인터넷 속성 대화 상자에 지정 된 대로 현재 검색 페이지로 이동 합니다.
...
지정 된 리소스를 WebBrowser 컨트롤에 로드 하려면이 멤버 함수를 호출 합니다.
...
URL로 식별 되는 리소스로 이동 하려면이 멤버 함수를 호출 합니다.
...
*URL*<br/>
탐색할 URL 또는 표시할 파일의 전체 경로를 포함 하는 호출자가 할당 한 문자열입니다.
...
*lpszHeaders*<br/>
서버에 보낼 HTTP 헤더를 지정 하는 값에 대 한 포인터입니다. 이러한 헤더는 기본 Internet Explorer 헤더에 추가 됩니다. 헤더는 서버에 필요한 작업, 서버로 전달 되는 데이터 형식 또는 상태 코드와 같은 항목을 지정할 수 있습니다. *Url* 이 HTTP url이 아니면이 매개 변수는 무시 됩니다.

*lpvPostData*<br/>
HTTP POST 트랜잭션과 함께 보낼 데이터에 대 한 포인터입니다. 예를 들어 POST 트랜잭션은 HTML 폼에 의해 수집 된 데이터를 전송 하는 데 사용 됩니다. 이 매개 변수가 post 데이터를 지정 하지 않으면에서 `Navigate` HTTP GET 트랜잭션을 발급 합니다. *Url* 이 HTTP url이 아니면이 매개 변수는 무시 됩니다.

*dwPostDataLen*<br/>
HTTP POST 트랜잭션과 함께 보낼 데이터입니다. 예를 들어 POST 트랜잭션은 HTML 폼에 의해 수집 된 데이터를 전송 하는 데 사용 됩니다. 이 매개 변수가 post 데이터를 지정 하지 않으면에서 `Navigate` HTTP GET 트랜잭션을 발급 합니다. *Url* 이 HTTP url이 아니면이 매개 변수는 무시 됩니다.
...
이 멤버 함수를 호출 하 여 URL로 식별 된 리소스 또는 전체 경로로 식별 된 파일로 이동 합니다.
...
*lpvPostData*<br/>
HTTP POST 트랜잭션과 함께 보낼 데이터입니다. 예를 들어 POST 트랜잭션은 HTML 폼에 의해 수집 된 데이터를 전송 하는 데 사용 됩니다. 이 매개 변수가 post 데이터를 지정 하지 않으면에서 `Navigate2` HTTP GET 트랜잭션을 발급 합니다. *Url* 이 HTTP 또는 HTTPS url이 아니면이 매개 변수는 무시 됩니다.

*dwPostDataLen*<br/>
*Lpvpostdata* 매개 변수가 가리키는 데이터의 길이 (바이트)입니다.

*lpszHeaders*<br/>
서버에 보낼 HTTP 또는 HTTPS 헤더를 지정 하는 값에 대 한 포인터입니다. 이러한 헤더는 기본 Internet Explorer 헤더에 추가 됩니다. 헤더는 서버에 필요한 작업, 서버로 전달 되는 데이터 형식 또는 상태 코드와 같은 항목을 지정할 수 있습니다. *Url* 이 HTTP 또는 HTTPS url이 아니면이 매개 변수는 무시 됩니다.
...
이 멤버 함수는 `Navigate` *pidl* 매개 변수로 표시 되는 데스크톱 및 내 컴퓨터 같은 특수 폴더에 대 한 검색을 지원 하 여 멤버 함수를 확장 합니다.
...
이 멤버 함수는 웹 브라우저에서 탐색이 발생 하기 전에 이벤트를 발생 시키는 프레임 워크에서 호출 됩니다.
...
*lpszTargetFrameName*<br/>
리소스를 표시할 프레임의 이름을 포함 하는 문자열 이거나, 리소스를 대상으로 하는 명명 된 프레임이 없는 경우 NULL입니다.

*baPostedData*<br/>
`CByteArray`HTTP POST 트랜잭션이 사용 되는 경우 서버에 보낼 데이터를 포함 하는 개체에 대 한 참조입니다.

*lpszHeaders*<br/>
서버에 보낼 추가 HTTP 헤더를 포함 하는 문자열에 대 한 포인터입니다 (HTTP Url에만 해당). 헤더는 서버에 필요한 작업, 서버로 전달 되는 데이터 형식 또는 상태 코드와 같은 항목을 지정할 수 있습니다.

*Pcancel*<br/>
취소 플래그에 대 한 포인터입니다. 응용 프로그램에서는이 매개 변수를 0이 아닌 값으로 설정 하 여 탐색 작업을 취소 하거나 0으로 설정 하 여 계속 진행할 수 있습니다.
...
*n*<br/>
사용 상태가 변경 된 명령의 식별자입니다.

*bEnable*<br/>
활성화 상태입니다. 명령을 사용 하는 경우이 매개 변수는 0이 아니며, 사용 하지 않도록 설정 된 경우 0입니다.

## <a name="chtmlviewondocumentcomplete"></a><a name="ondocumentcomplete"></a> CHtmlView:: OnDocumentComplete

이 멤버 함수는 문서가 READYSTATE_COMPLETE 상태에 도달 했음을 응용 프로그램에 알리기 위해 프레임 워크에서 호출 됩니다.
...
*lpszURL*<br/>
URL, UNC 파일 이름 또는로 탐색 된 PIDL (항목 식별자 목록에 대 한 포인터)로 계산 되는 문자열에 대 한 포인터입니다.

### <a name="remarks"></a>설명

모든 프레임에서이 이벤트를 발생 시키는 것은 아니지만 [Ondownloadbegin](#ondownloadbegin) 이벤트를 발생 시키는 각 프레임은 해당 이벤트를 발생 시킵니다 `OnDocumentComplete` .

이 URL은 정규화 되 고 정규화 된 URL 이기 때문에 *lpszURL* 로 표시 되는 url은 브라우저가 탐색 하도록 지시한 url과 다를 수 있습니다. 예를 들어 응용 프로그램에서 `"www.microsoft.com"` [Navigate](#navigate) 또는 [Navigate2](#navigate2)호출에 url을 지정 하는 경우에서 전달 하는 url은가 됩니다 `OnNavigateComplete2` `"<https://www.microsoft.com/>"` . 또한 서버에서 브라우저를 다른 URL로 리디렉션한 경우 리디렉션된 URL이 여기에 반영 됩니다.

## <a name="chtmlviewondocwindowactivate"></a><a name="ondocwindowactivate"></a> CHtmlView:: OnDocWindowActivate

의 Internet Explorer 또는 MSHTML 구현에서 호출 `IOleInPlaceActiveObject::OnDocWindowActivate` 되며, 컨테이너의 문서 창이 활성화 또는 비활성화 되 면 활성 내부 개체에 알립니다.
...
*fActivate*<br/>
문서 창의 상태를 나타냅니다. 이 값이 0이 아니면 창이 활성화 되 고 있는 것입니다. 이 값이 0 이면 창을 비활성화 하는 것입니다.

### <a name="return-value"></a>반환 값

성공 하면 S_OK이 고, 그렇지 않으면 OLE 정의 오류 코드입니다.

### <a name="remarks"></a>설명

`OnDocWindowActivate` `OnDocWindowActivate` Microsoft 웹 브라우저 컨트롤의 알림에 반응 하도록 재정의 합니다. 자세한 내용은 Windows SDK에서 [IDocHostUIHandler:: OnDocWindowActivate](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753261\(v=vs.85\)) 를 참조 하세요.

## <a name="chtmlviewondownloadbegin"></a><a name="ondownloadbegin"></a> CHtmlView:: OnDownloadBegin

이 멤버 함수는 문서 다운로드를 시작 하기 위해 프레임 워크에서 호출 됩니다.
...
탐색이 취소 되지 않는 한이 이벤트는 [OnBeforeNavigate2](#onbeforenavigate2) 이벤트 직후에 발생 합니다. 모든 애니메이션 또는 "사용 중" 표시는 컨테이너를 표시 해야 함을이 이벤트에 연결 해야 함을 나타냅니다.

## <a name="chtmlviewondownloadcomplete"></a><a name="ondownloadcomplete"></a> CHtmlView:: OnDownloadComplete

이 멤버 함수는 탐색 작업이 완료, 중단 또는 실패 했음을 나타내기 위해 프레임 워크에서 호출 됩니다.
...
Internet Explorer 또는 MSHTML에서 모달 UI를 표시할 때 호출 됩니다.
...
*fEnable*<br/>
호스트의 모덜리스 대화 상자를 사용할 수 있는지 여부를 나타냅니다. 이 값이 0이 아니면 모덜리스 대화 상자를 사용할 수 있습니다. 이 값이 0 이면 모덜리스 대화 상자를 사용할 수 없습니다.
...
컨테이너가 모달 대화 상자를 만들거나 삭제할 때 모덜리스 대화 상자를 사용 하거나 사용 하지 않도록 설정 합니다. `OnEnableModeless` `EnableModeless` Microsoft 웹 브라우저 컨트롤의 알림에 반응 하도록 재정의 합니다. 자세한 내용은 Windows SDK에서 [IDocHostUIHandler:: EnableModeless](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753253\(v=vs.85\)) 를 참조 하세요.
...
*ppDataObject*<br/>
호스트에서 제공 하는 인터페이스 포인터를 수신 하는 주소입니다 `IDataObject` . 메서드가 실패 하는 경우에도이 매개 변수의 내용은 항상 NULL로 초기화 되어야 합니다.

### <a name="return-value"></a>반환 값

데이터 개체가 대체 되 면이 고, 그렇지 않으면 데이터 개체가 교체 되지 않은 경우 S_FALSE S_OK, 오류가 발생 하는 경우 OLE 정의 오류 코드입니다.

### <a name="remarks"></a>설명

`OnFilterDataObject` `FilterDataObject` Microsoft 웹 브라우저 컨트롤의 알림에 반응 하도록 재정의 합니다. 자세한 내용은 Windows SDK에서 [IDocHostUIHandler:: FilterDataObject](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753254\(v=vs.85\)) 를 참조 하세요.

## <a name="chtmlviewonframewindowactivate"></a><a name="onframewindowactivate"></a> CHtmlView:: On프레임 창 활성화
...
*fActivate*<br/>
컨테이너의 최상위 프레임 창의 상태를 나타냅니다. 이 값이 0이 아니면 창이 활성화 되 고 있는 것입니다. 이 값이 0 이면 창을 비활성화 하는 것입니다.
...
`OnFrameWindowActivate` `OnFrameWindowActivate` Microsoft 웹 브라우저 컨트롤의 알림에 반응 하도록 재정의 합니다. 자세한 내용은 Windows SDK에서 [IDocHostUIHandler:: On Windowactivate](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753262\(v=vs.85\)) 를 참조 하세요.

## <a name="chtmlviewonfullscreen"></a><a name="onfullscreen"></a> CHtmlView:: OnFullScreen 화면

이 멤버 함수는 [전체 화면](/previous-versions/aa752119\(v=vs.85\)) 속성이 변경 될 때 프레임 워크에서 호출 됩니다.
...
*bFullScreen 화면*<br/>
Internet Explorer가 전체 화면 모드에 있는 경우 0이 아님 그렇지 않으면 0입니다.
...
*ppDropTarget*<br/>
`IDropTarget` `IDropTarget` 호스트에서 제공 하고자 하는 인터페이스 포인터를 수신 하는의 주소입니다.

### <a name="return-value"></a>반환 값

반환 코드 목록은 Windows SDK의 [IDocHostUIHandler:: GetDropTarget](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753255\(v=vs.85\)) 을 참조 하세요.

### <a name="remarks"></a>설명

`OnGetDropTarget` `GetDropTarget` Microsoft 웹 브라우저 컨트롤의 알림에 반응 하도록 재정의 합니다. 자세한 내용은 Windows SDK의 [IDocHostUIHandler:: GetDropTarget](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753255\(v=vs.85\)) (영문)을 참조 하세요.
...
*lppDispatch*<br/>
`IDispatch`호스트 응용 프로그램의 인터페이스 포인터를 받는 주소에 대 한 포인터입니다. 호스트가 자동화 인터페이스를 노출 하는 경우이 매개 변수를 통해 Internet Explorer 또는 MSHTML에 대 한 참조를 제공할 수 있습니다. 메서드가 실패 하는 경우에도이 매개 변수의 내용은 항상 NULL로 초기화 되어야 합니다.
...
`OnGetExternal` `GetExternal` Microsoft 웹 브라우저 컨트롤의 알림에 반응 하도록 재정의 합니다. 자세한 내용은 Windows SDK에서 [IDocHostUIHandler:: GetExternal](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753256\(v=vs.85\)) 를 참조 하세요.
...
*pInfo*<br/>
호스트의 UI 기능을 수신 하는 [DOCHOSTUIINFO](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa770044\(v=vs.85\)) 구조체의 주소입니다.
...
`OnGetHostInfo` `GetHostInfo` Microsoft 웹 브라우저 컨트롤의 알림에 반응 하도록 재정의 합니다. 자세한 내용은 Windows SDK에서 [IDocHostUIHandler:: GetHostInfo](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753257\(v=vs.85\)) 를 참조 하세요.

## <a name="chtmlviewongetoptionkeypath"></a><a name="ongetoptionkeypath"></a> CHtmlView:: OnGetOptionKeyPath

Internet Explorer 또는 MSHTML이 사용자 기본 설정을 저장 하는 레지스트리 키를 가져오려면이 멤버 함수를 호출 합니다.
...
성공 하면 S_OK이 고, 그렇지 않으면 S_FALSE입니다. S_FALSE 경우 Internet Explorer 또는 MSHTML은 기본적으로 자체 사용자 옵션을 사용 합니다.

### <a name="remarks"></a>설명

`OnGetOptionKeyPath` `GetOptionKeyPath` Microsoft 웹 브라우저 컨트롤의 알림에 반응 하도록 재정의 합니다. 자세한 내용은 Windows SDK에서 [IDocHostUIHandler:: GetOptionKeyPath](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753258\(v=vs.85\)) 를 참조 하세요.

## <a name="chtmlviewonhideui"></a><a name="onhideui"></a> CHtmlView:: OnHideUI

이 멤버 함수는 Internet Explorer 또는 MSHTML에서 메뉴와 도구 모음을 제거할 때 프레임 워크에서 호출 됩니다.
...
`OnHideUI` `HideUI` Microsoft 웹 브라우저 컨트롤의 알림에 반응 하도록 재정의 합니다. 자세한 내용은 Windows SDK에서 [IDocHostUIHandler:: HideUI](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753259\(v=vs.85\)) 를 참조 하세요.

## <a name="chtmlviewonmenubar"></a><a name="onmenubar"></a> CHtmlView:: OnMenuBar

이 멤버 함수는 [MenuBar](/previous-versions/aa752131\(v=vs.85\)) 속성이 변경 될 때 프레임 워크에서 호출 됩니다.
...
*bMenuBar*<br/>
Internet Explorer 메뉴 모음이 표시 되는 경우 0이 아닙니다. 그렇지 않으면 0입니다.

## <a name="chtmlviewonnavigatecomplete2"></a><a name="onnavigatecomplete2"></a> CHtmlView:: OnNavigateComplete2

이 멤버 함수는 창 또는 프레임셋 요소에서 하이퍼링크가 완료 된 후 프레임 워크에서 호출 됩니다.
...
*strURL*<br/>
URL, UNC 파일 이름 또는 PIDL (항목 식별자 목록에 대 한 포인터)로 계산 되는 문자열 식입니다.

### <a name="remarks"></a>설명

Url 매개 변수는 URL 표시가 없는 셸 이름 공간 엔터티의 경우 PIDL 일 수 있습니다.

*StrURL* 에 포함 된 url은 브라우저가 탐색 하도록 지시한 url과 다를 수 있습니다 .이 url은 정규화 된 url 이며 정규화 된 url입니다. 예를 들어 응용 프로그램에서 `"www.microsoft.com"` [Navigate](#navigate) 또는 [Navigate2](#navigate2)호출에 url을 지정 하는 경우에서 전달 하는 url은가 됩니다 `OnNavigateComplete2` `"<https://www.microsoft.com/>"` . 또한 서버에서 브라우저를 다른 URL로 리디렉션한 경우 리디렉션된 URL이 여기에 반영 됩니다.
...
*lpszFrame*<br/>
리소스를 표시할 프레임의 이름 이거나, 리소스를 대상으로 하는 명명 된 프레임이 없는 경우 NULL입니다.

*dwError*<br/>
사용할 수 있는 경우 오류 상태 코드입니다. 가능한 HRESULT 및 HTTP 상태 코드 목록은 [NavigateError 이벤트 상태 코드](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa768365\(v=vs.85\))를 참조 하세요.

*Pcancel*<br/>
오류 페이지나 추가 자동 검색에 대 한 탐색을 취소할지 여부를 지정 합니다. TRUE (기본값) 이면 오류 페이지 또는 자동 검색에 대 한 탐색을 계속 합니다. FALSE 이면 오류 페이지 또는 자동 검색에 대 한 탐색을 취소 합니다.

### <a name="remarks"></a>설명

사용자 지정 탐색 오류 처리를 제공 하려면이 메서드를 재정의 합니다.

자세한 내용은 [DWebBrowserEvents2:: NavigateError](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa768286\(v=vs.85\)) 를 참조 하세요.

## <a name="chtmlviewonnewwindow2"></a><a name="onnewwindow2"></a> CHtmlView:: OnNewWindow2

이 멤버 함수는 리소스를 표시 하기 위해 새 창을 만들 때 프레임 워크에서 호출 됩니다.
...
*ppDisp*<br/>
`IDispatch`새 WebBrowser 또는 Internet Explorer 개체의 인터페이스 포인터를 선택적으로 받을 수 있는 인터페이스 포인터에 대 한 포인터입니다.
...
이 이벤트는 WebBrowser 내에서 새 창을 만드는 것 보다 앞에 옵니다.
...
컨테이너는이 이벤트에서 제공 하는 정보를 사용 하 여 지금까지 다운로드 한 바이트 수를 표시 하거나 진행률 표시기를 업데이트할 수 있습니다.

## <a name="chtmlviewonpropertychange"></a><a name="onpropertychange"></a> CHtmlView:: OnPropertyChange

이 멤버 함수는 [Putproperty](#putproperty) 가 속성 값을 변경 했음을 응용 프로그램에 알리기 위해 프레임 워크에서 호출 됩니다.
...
이 멤버 함수는 Internet Explorer 응용 프로그램을 종료할 준비가 되었음을 응용 프로그램에 알리기 위해 프레임 워크에서 호출 됩니다.
...
`OnResizeBorder` `ResizeBorder` Microsoft 웹 브라우저 컨트롤의 알림에 반응 하도록 재정의 합니다. 자세한 내용은 Windows SDK에서 [IDocHostUIHandler:: ResizeBorder](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753263\(v=vs.85\)) 를 참조 하세요.
...
*dwID*<br/>
표시 되는 상황에 맞는 메뉴의 식별자입니다. `IDocHostUIHandler::ShowContextMenu`값 목록은 Windows SDK에서를 참조 하세요.
...
값 목록은 Windows SDK의 [IDocHostUIHandler:: ShowContextMenu](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753264\(v=vs.85\)) 를 참조 하세요.

### <a name="remarks"></a>설명

`OnShowContextMenu` `ShowContextMenu` Microsoft 웹 브라우저 컨트롤의 알림에 반응 하도록 재정의 합니다. 자세한 내용은 Windows SDK에서 [IDocHostUIHandler:: ShowContextMenu](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753264\(v=vs.85\)) 를 참조 하세요.
...
값 목록은 Windows SDK의 [IDocHostUIHandler:: ShowUI](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753265\(v=vs.85\)) 를 참조 하세요.

### <a name="remarks"></a>설명

`OnShowUI` `ShowUI` Microsoft 웹 브라우저 컨트롤의 알림에 반응 하도록 재정의 합니다. 자세한 내용은 Windows SDK에서 [IDocHostUIHandler:: ShowUI](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753265\(v=vs.85\)) 를 참조 하세요.

## <a name="chtmlviewonstatusbar"></a><a name="onstatusbar"></a> CHtmlView:: OnStatusBar

이 멤버 함수는 [StatusBar](/previous-versions//aa768270\(v=vs.85\)) 속성이 변경 될 때 프레임 워크에서 호출 됩니다.
...
*bStatusBar*<br/>
Internet Explorer의 상태 표시줄이 표시 되 면 0이 아닌 값이 고, 그렇지 않으면 0입니다.
...
이 멤버 함수는 [TheaterMode](/previous-versions/aa768273\(v=vs.85\)) 속성이 변경 될 때 프레임 워크에서 호출 됩니다.
...
*bTheaterMode*<br/>
Internet Explorer가 극장 모드 이면 0이 아닌 것입니다. 그렇지 않으면 0입니다.

## <a name="chtmlviewontitlechange"></a><a name="ontitlechange"></a> CHtmlView:: OnTitleChange

이 멤버 함수는 WebBrowser 컨트롤의 문서 제목을 사용할 수 있게 되거나 변경 된 경우 응용 프로그램에 알리기 위해 프레임 워크에서 호출 됩니다.
...
HTML의 경우 제목이 변경 될 수 있습니다. HTML을 계속 다운로드 하는 동안에는 문서의 URL이 제목으로 설정 됩니다. HTML에서 실제 제목 (있는 경우)이 구문 분석 되 면 제목이 실제 제목을 반영 하도록 변경 됩니다.

## <a name="chtmlviewontoolbar"></a><a name="ontoolbar"></a> CHtmlView:: OnToolBar

이 멤버 함수는 [ToolBar](/previous-versions/aa768274\(v=vs.85\)) 속성이 변경 될 때 프레임 워크에서 호출 됩니다.
...
*bToolBar*<br/>
Internet Explorer의 도구 모음이 표시 되 면 0이 아닌 값이 고, 그렇지 않으면 0입니다.
...
`OnTranslateAccelerator` `TranslateAccelerator` Microsoft 웹 브라우저 컨트롤의 알림에 반응 하도록 재정의 합니다. 자세한 내용은 Windows SDK에서 [IDocHostUIHandler:: TranslateAccelerator](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753266\(v=vs.85\)) 를 참조 하세요.
...
*pchURLIn*<br/>
번역할 URL을 나타내는 Internet Explorer 또는 MSHTML에서 제공 하는 문자열의 주소입니다.

*ppchURLOut*<br/>
번역 된 URL의 주소를 받는 문자열 포인터의 주소입니다. 호스트는 작업 메모리 할당자를 사용 하 여 버퍼를 할당 합니다. URL이 변환 되지 않거나 메서드가 실패할 경우에도이 매개 변수의 내용은 항상 NULL로 초기화 되어야 합니다.

### <a name="return-value"></a>반환 값

`S_OK` URL이 변환 된 경우 URL이 변환 되지 않은 경우 S_FALSE이 고, 오류가 발생 한 경우 OLE 정의 오류 코드입니다.

### <a name="remarks"></a>설명

`OnTranslateUrl` `TranslateUrl` Microsoft 웹 브라우저 컨트롤의 알림에 반응 하도록 재정의 합니다. 자세한 내용은 Windows SDK에서 [IDocHostUIHandler:: TranslateUrl](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753267\(v=vs.85\)) 를 참조 하세요.
...
호스트에서 도구 모음 단추의 상태를 업데이트 해야 합니다. 이 메서드는의 반환 값에 관계 없이 호출 됩니다 `ShowUI` . `OnUpdateUI` `UpdateUI` Microsoft 웹 브라우저 컨트롤의 알림에 반응 하도록 재정의 합니다.

## <a name="chtmlviewonvisible"></a><a name="onvisible"></a> CHtmlView:: OnVisible

이 멤버 함수는 WebBrowser의 창이 표시 되거나 숨겨질 때 프레임 워크에서 호출 됩니다.
...
*bVisible*<br/>
개체가 표시 되 면 0이 아닌 값이 고, 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

그러면 Internet Explorer 창이 동작 하는 것과 동일한 방식으로 개체 컨트롤 호스트 창이 동작할 수 있습니다.

## <a name="chtmlviewputproperty"></a><a name="putproperty"></a> CHtmlView::P utProperty

지정 된 개체와 연결 된 속성을 설정 하려면이 멤버 함수를 호출 합니다.
...
*vtValue*<br/>
*LpszProperty* 로 표시 된 속성의 새 값입니다.
...
## <a name="chtmlviewqueryformscommand"></a><a name="queryformscommand"></a> CHtmlView:: Query Command
...
*Psupported*<br/>
*DwCommandID* 로 식별 되는 명령이 지원 되는지 여부를 지정 하는 부울에 대 한 포인터입니다. TRUE 이면 명령이 지원 됩니다. 그렇지 않으면 FALSE입니다.

*Penabled*<br/>
명령 ( *dwCommandID* 로 식별 됨)이 활성화 되어 있는지 여부를 지정 하는 부울에 대 한 포인터입니다. TRUE 이면 명령이 지원 됩니다. 그렇지 않으면 FALSE입니다.

*Pchecked*<br/>
명령 ( *dwCommandID* 로 식별 됨)이 선택 되어 있는지 여부를 지정 하는 부울에 대 한 포인터입니다. TRUE 이면 명령이 지원 됩니다. 그렇지 않으면 FALSE입니다.
...
웹 브라우저가 현재 표시 하는 URL 또는 파일을 다시 로드 합니다.
...
`Refresh` 새로 고침 수준을 설정 하기 위한 매개 변수를 포함 하지 않습니다.

Internet Explorer 및 WebBrowser에 적용됩니다.

## <a name="chtmlviewrefresh2"></a><a name="refresh2"></a> CHtmlView:: Refresh2

Internet Explorer에서 현재 표시 하 고 있는 파일을 다시 로드 합니다.
...
*n 수준*<br/>
새로 고침 수준을 지정 하는 변수의 주소입니다. 가능한 변수는 Windows SDK의 [Refreshconstants](/previous-versions/aa768363\(v=vs.85\))에 정의 되어 있습니다.

### <a name="remarks"></a>설명

[Refresh](#refresh)와 달리 `Refresh2` 새로 고침 수준을 지정 하는 매개 변수를 포함 합니다.

Internet Explorer 및 WebBrowser에 적용됩니다.

## <a name="chtmlviewsetaddressbar"></a><a name="setaddressbar"></a> CHtmlView:: SetAddressBar

Internet Explorer 개체의 주소 표시줄을 표시 하거나 숨기려면이 멤버 함수를 호출 합니다.
...
*bNewValue*<br/>
0이 아닌 주소 표시줄을 표시 합니다. 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

Internet Explorer에 적용 됩니다. WebBrowser 컨트롤과 함께이 호출을 사용 하는 경우 오류를 반환 하지 않지만이 호출을 무시 합니다.

## <a name="chtmlviewsetfullscreen"></a><a name="setfullscreen"></a> CHtmlView:: SetFullScreen 화면

Internet Explorer를 전체 화면 모드 또는 표준 창 모드로 설정 하려면이 멤버 함수를 호출 합니다.
...
*bNewValue*<br/>
전체 화면 모드의 경우 0이 아닙니다. 그렇지 않으면 0입니다.
...
Internet Explorer 주 창의 높이를 설정 하려면이 멤버 함수를 호출 합니다.
...
*nNewValue*<br/>
주 창 왼쪽 가장자리의 화면 좌표입니다.

## <a name="chtmlviewsetmenubar"></a><a name="setmenubar"></a> CHtmlView:: SetMenuBar

Internet Explorer 메뉴 모음을 표시 하거나 숨기려면이 멤버 함수를 호출 합니다.
...
*bNewValue*<br/>
0이 아닌 메뉴 모음을 표시 합니다. 그렇지 않으면 0입니다.
...
WebBrowser 컨트롤이 현재 오프 라인 모드에서 작동 하는지 여부를 나타내는 값을 설정 하려면이 멤버 함수를 호출 합니다.
...
*bNewValue*<br/>
0이 아닌 값은 로컬 캐시에서 읽습니다. 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

오프 라인 모드에서는 브라우저가 원본 문서가 아닌 로컬 캐시에서 HTML 페이지를 읽습니다.

Internet Explorer 및 WebBrowser에 적용됩니다.

## <a name="chtmlviewsetregisterasbrowser"></a><a name="setregisterasbrowser"></a> CHtmlView:: SetRegisterAsBrowser

이 멤버 함수를 호출 하 여 WebBrowser 컨트롤이 대상 이름 확인을 위한 최상위 브라우저로 등록 되었는지 여부를 나타내는 값을 설정 합니다.
...
*bNewValue*<br/>
Internet Explorer가 최상위 브라우저로 등록 되었는지 여부를 확인 합니다. 0이 아니면 웹 브라우저가 최상위 브라우저로 등록 됩니다. 0 인 경우 최상위 브라우저가 아닙니다. 기본값은 영입니다.

### <a name="remarks"></a>설명

최상위 브라우저는 레지스트리에 기본 브라우저로 설정 된 브라우저입니다.
...
*bNewValue*<br/>
WebBrowser 컨트롤이 탐색을 위한 놓기 대상으로 등록 되었는지 여부를 확인 합니다. 0이 아닌 경우 개체가 놓기 대상으로 등록 됩니다. 0 인 경우 놓기 대상이 아닙니다.
...
대화 상자를 표시할 수 있는지 여부를 나타내는 값을 설정 하려면이 멤버 함수를 호출 합니다.
...
*bNewValue*<br/>
0이 아닌 경우 대화 상자는 표시 되지 않습니다. 0 인 경우 대화 상자가 표시 됩니다. 기본값은 영입니다.
...
*bNewValue*<br/>
상태 표시줄이 표시 되 면 0이 아닌 값입니다. 그렇지 않으면 0입니다.
...
*bNewValue*<br/>
0이 아니면 WebBrowser 컨트롤을 극장 모드로 설정 합니다. 그렇지 않으면 0입니다. 기본값은 영입니다.
...
Internet Explorer 도구 모음을 표시 하거나 숨기려면이 멤버 함수를 호출 합니다.
...
*nNewValue*<br/>
도구 모음을 표시할지 여부를 나타냅니다. 도구 모음이 표시 되 면 0이 아닌 값입니다. 그렇지 않으면 0입니다.
...
이 멤버 함수를 호출 하 여 WebBrowser 컨트롤의 내부 위쪽 가장자리와 해당 컨테이너의 위쪽 가장자리 사이의 거리를 설정 합니다.
...
이 멤버 함수를 호출 하 여 WebBrowser 컨트롤의 표시 상태를 설정 합니다.
...
*bNewValue*<br/>
컨트롤이 표시 되는 경우 0이 아닙니다. 그렇지 않으면 0입니다.
...
보류 중인 탐색 또는 다운로드 작업을 취소 하 고 배경 소리 및 애니메이션과 같은 모든 동적 페이지 요소를 중지 하려면이 멤버 함수를 호출 합니다.
