---
description: '자세히 알아보기: CWinAppEx 클래스'
title: CWinAppEx 클래스
ms.date: 11/04/2016
f1_keywords:
- CWinAppEx
...
ms.locfileid: "97318457"
---
# <a name="cwinappex-class"></a>CWinAppEx 클래스

`CWinAppEx` 응용 프로그램 상태를 처리 하 고, 상태를 레지스트리에 저장 하 고, 레지스트리에서 상태를 로드 하 고, 응용 프로그램 관리자를 초기화 하 고, 동일한 응용 프로그램 관리자에 대 한 링크를 제공 합니다.
...
|[CWinAppEx:: ExitInstance](#exitinstance)|`Run`응용 프로그램의이 인스턴스를 종료 하기 위해 멤버 함수 내에서 프레임 워크에 의해 호출 됩니다. ( [CWinApp:: ExitInstance](../../mfc/reference/cwinapp-class.md#exitinstance)를 재정의 합니다.)|
...
|[CWinAppEx:: GetObject](#getobject)|읽기 `CObject` -레지스트리에서 지정 된 값과 연결 된 파생 된 데이터입니다.|
...
|[CWinAppEx:: Get섹션 개체](#getsectionobject)|`CObject`레지스트리에서 지정 된 키와 값이 연결 된 데이터를 읽습니다.|
...
|[CWinAppEx:: InitShellManager](#initshellmanager)|클래스를 초기화 합니다. `CShellManager`|
...
|[CWinAppEx:: WriteSectionObject](#writesectionobject)|클래스에서 파생 된 데이터 `CObject` 를 지정 된 레지스트리 키의 값에 씁니다.|
...
이 메서드는 레지스트리의 특정 섹션에서 응용 프로그램 데이터를 지웁니다. *LpszSectionName* 매개 변수를 사용 하 여 지울 섹션을 지정할 수 있습니다. *LpszSectionName* 가 NULL 인 경우이 메서드는 개체에 저장 된 기본 레지스트리 경로를 사용 합니다 `CWinAppEx` . 기본 레지스트리 경로를 가져오려면 [CWinAppEx:: GetRegistryBase](#getregistrybase)를 사용 합니다.
...
`CWinAppEx`클래스는 초기화 메서드를 포함 하며, 응용 프로그램 정보를 레지스트리에 저장 하 고 로드 하 고 전역 응용 프로그램 설정을 제어 하는 기능을 제공 합니다. 또한 [CKeyboardManager 클래스](../../mfc/reference/ckeyboardmanager-class.md) 및 [CUserToolsManager 클래스](../../mfc/reference/cusertoolsmanager-class.md)와 같은 전역 관리자를 사용할 수 있습니다. 각 응용 프로그램은 클래스의 인스턴스를 하나만 포함할 수 있습니다 `CWinAppEx` .
...
`CMenuTearOffManager`가 만들어지고 성공적으로 초기화 되 면 TRUE이 고, 그렇지 않으면입니다. 오류가 발생 하거나가 이미 있으면 FALSE `CMenuTearOffManager` 입니다.

### <a name="remarks"></a>설명

이 함수를 사용 하 여 응용 프로그램에서 분리 메뉴를 사용 하도록 설정 합니다. 에서이 함수를 호출 해야 `InitInstance` 합니다.
...
*pToolRTC*<br/>
진행 `CUserToolsManager` 개체에서 새 사용자 도구를 만드는 데 사용 하는 클래스입니다.
...
메서드가 개체를 만들고 초기화 하는 경우 TRUE `CUserToolsManager` 입니다. 메서드가 실패 하거나 `CUserToolsManager` 개체가 이미 있으면 FALSE입니다.
...
전역 개체에 대 한 포인터 `CContextMenuManager` 입니다.
...
전역 개체에 대 한 포인터 `CKeyboardManager` 입니다.
...
전역 개체에 대 한 포인터 `CMouseManager` 입니다.
...
*obj*<br/>
제한이 에 대 한 참조 `CObject` 입니다. 메서드는이 참조를 사용 하 여 레지스트리 데이터를 저장 합니다.
...
이 메서드는에서 파생 된 레지스트리에서 데이터를 읽습니다 `CObject` . `CObject`레지스트리에 데이터를 쓰려면 [CWinAppEx:: WriteObject](#writeobject) 또는 [CWinAppEx:: WriteSectionObject](#writesectionobject)중 하나를 사용 합니다.
...
## <a name="cwinappexgetregsectionpath"></a><a name="getregsectionpath"></a> CWinAppEx:: Getreg섹션 경로
...
`CString`레지스트리 키의 절대 경로를 포함 하는입니다.
...
## <a name="cwinappexgetsectionbinary"></a><a name="getsectionbinary"></a> CWinAppEx:: Get섹션 이진
...
## <a name="cwinappexgetsectionint"></a><a name="getsectionint"></a> CWinAppEx:: Get섹션 Int
...
## <a name="cwinappexgetsectionobject"></a><a name="getsectionobject"></a> CWinAppEx:: Get섹션 개체
...
*obj*<br/>
제한이 에 대 한 참조 `CObject` 입니다. 메서드는이를 사용 하 여 `CObject` 레지스트리 데이터를 저장 합니다.
...
이 메서드는 레지스트리에서 데이터를 읽습니다. 데이터 읽기는 `CObject` 데이터 이거나에서 파생 된 클래스의 데이터입니다 `CObject` . `CObject`레지스트리에 데이터를 쓰려면 [CWinAppEx:: WriteObject](#writeobject) 또는 [CWinAppEx:: WriteSectionObject](#writesectionobject)중 하나를 사용 합니다.

*LpszSubSection* 매개 변수는 레지스트리 항목의 절대 경로가 아닙니다. 응용 프로그램에 대 한 기본 레지스트리 키의 끝에 추가 되는 상대 경로입니다. 기본 레지스트리 키를 가져오거나 설정 하려면 [CWinAppEx:: GetRegistryBase](#getregistrybase) 및 [CWinAppEx:: SetRegistryBase](#setregistrybase) 메서드를 각각 사용 합니다.

## <a name="cwinappexgetsectionstring"></a><a name="getsectionstring"></a> CWinAppEx:: Get섹션 문자열
...
전역 개체에 대 한 포인터 `CShellManager` 입니다.
...
전역 개체에 대 한 포인터 `CTooltipManager` 입니다.
...
전역 개체에 대 한 포인터 `CUserToolsManager` 입니다. 응용 프로그램에 대해 사용자 도구 관리를 사용 하도록 설정 하지 않은 경우 NULL입니다.

### <a name="remarks"></a>설명

개체에 대 한 포인터를 검색 하기 전에 `CUserToolsManager` [CWinAppEx:: EnableUserTools](#enableusertools)를 호출 하 여 관리자를 초기화 해야 합니다.
...
[CWinAppEx:: GetContextMenuManager](#getcontextmenumanager)를 호출 하는 경우 해당 메서드의 기본 구현에서는를 호출 `InitContextMenuManager` 합니다.

응용 프로그램에 상황에 맞는 메뉴 관리자가 이미 있고를 호출 하는 경우 `InitContextMenuManager` 응용 프로그램에 [ASSERT](diagnostic-services.md#assert) 오류가 발생 합니다. 따라서 `InitContextMenuManager` 개체를 직접 만드는 경우를 호출 하면 안 됩니다 `CContextMenuManager` . 사용자 지정를 사용 하지 않는 경우를 `CContextMenuManager` 사용 하 여 `GetContextMenuManager` 개체를 만들어야 합니다 `CContextMenuManager` .
...
메서드가 개체를 만드는 경우 0이 아닌 `CKeyboardManager` 값이 고 `CKeyboardManager` 개체가 이미 있으면 0입니다.

### <a name="remarks"></a>설명

[CWinAppEx:: GetKeyboardManager](#getkeyboardmanager)를 호출 하는 경우 해당 메서드의 기본 구현에서는를 호출 `InitKeyboardManager` 합니다.

응용 프로그램에 이미 키보드 관리자가 있고를 호출 하는 경우 `InitKeyboardManager` 응용 프로그램에 [ASSERT](diagnostic-services.md#assert) 오류가 발생 합니다. 따라서 `InitKeyboardManager` 개체를 직접 만드는 경우를 호출 하면 안 됩니다 `CKeyboardManager` . 사용자 지정를 사용 하지 않는 경우를 `CKeyboardManager` 사용 하 여 `GetKeyboardManager` 개체를 만들어야 합니다 `CKeyboardManager` .
...
메서드가 개체를 만드는 경우 0이 아닌 `CMouseManager` 값이 고 `CMouseManager` 개체가 이미 있으면 0입니다.

### <a name="remarks"></a>설명

[CWinAppEx:: GetMouseManager](#getmousemanager)를 호출 하는 경우 해당 메서드의 기본 구현에서는를 호출 `InitMouseManager` 합니다.

응용 프로그램에 이미 마우스 관리자가 있고를 호출 하는 경우 `InitMouseManager` 응용 프로그램에 [ASSERT](diagnostic-services.md#assert) 오류가 발생 합니다. 따라서 `InitMouseManager` 개체를 직접 만드는 경우를 호출 하면 안 됩니다 `CMouseManager` . 사용자 지정를 사용 하지 않는 경우를 `CMouseManager` 사용 하 여 `GetMouseManager` 개체를 만들어야 합니다 `CMouseManager` .
...
메서드가 개체를 만드는 경우 0이 아닌 `CShellManager` 값이 고 `CShellManager` 개체가 이미 있으면 0입니다.

### <a name="remarks"></a>설명

[CWinAppEx:: GetShellManager](#getshellmanager)를 호출 하는 경우 해당 메서드의 기본 구현에서는를 호출 `InitShellManager` 합니다.

응용 프로그램에 셸 관리자가 이미 있는 경우를 호출 하면 `InitShellManager` 응용 프로그램에서 [어설션](diagnostic-services.md#assert) 오류가 발생 합니다. 따라서 `InitShellManager` 개체를 직접 만드는 경우에는를 호출 하지 마세요 `CShellManager` . 사용자 지정를 사용 하지 않는 경우를 `CShellManager` 사용 `GetShellManager` 하 여 개체를 만듭니다 `CShellManager` .
...
메서드가 개체를 만드는 경우 0이 아닌 `CTooltipManager` 값이 고 `CTooltipManager` 개체가 이미 있으면 0입니다.

### <a name="remarks"></a>설명

[CWinAppEx:: GetTooltipManager](#gettooltipmanager)를 호출 하는 경우 해당 메서드의 기본 구현에서는를 호출 `InitTooltipManager` 합니다.

응용 프로그램에 도구 설명 관리자가 이미 있고를 호출 하는 경우 `InitTooltipManager` 응용 프로그램에 [ASSERT](diagnostic-services.md#assert) 오류가 발생 합니다. 따라서 `InitTooltipManager` 개체를 직접 만드는 경우를 호출 하면 안 됩니다 `CTooltipManager` . 사용자 지정를 사용 하지 않는 경우를 `CTooltipManager` 사용 하 여 `GetTooltipManager` 개체를 만들어야 합니다 `CTooltipManager` .
...
*P프레임의 구현이*<br/>
진행 개체에 대 한 포인터 `CFrameImpl` 입니다. 메서드는 레지스트리의 상태 정보를이 프레임 창에 적용 합니다.
...
호출의 기본 구현 `CFrameImpl::OnLoadFrame` `LoadState` 입니다.
...
*P프레임의 구현이*<br/>
진행 개체에 대 한 포인터 `CFrameImpl` 입니다. 이 프레임은 Windows 레지스트리에 저장 됩니다.
...
팝업 메뉴를 지원 하려면 [CContextMenuManager](../../mfc/reference/ccontextmenumanager-class.md) 개체가 있어야 합니다. 개체를 초기화 하지 않은 경우 `CContextMenuManager` 에 `ShowPopupMenu` 는이 실패 합니다.

## <a name="cwinappexstorewindowplacement"></a><a name="storewindowplacement"></a> CWinAppEx:: Windowplacement
...
*obj*<br/>
진행 메서드에서 저장할 데이터에 대 한 참조 `CObject` 입니다.
