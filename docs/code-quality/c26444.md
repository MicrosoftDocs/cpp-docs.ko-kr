---
description: '자세히 알아보기: C26444 NO_UNNAMED_RAII_OBJECTS'
title: C26444
keywords: C26444
ms.date: 01/18/2017
ms.topic: reference
...
- 표준 라이브러리 구현에는 일부 형식 (예: 컨테이너)에 대해 서로 다른 버전의 소멸 논리가 있을 수 있습니다. 이는 [std:: vector:: insert](../standard-library/vector-class.md#insert)와 같은 호출에서 반환 된 반복기를 무시 하기 때문에 디버그 빌드에 대 한 잡음이 있는 경고를 생성할 수 있습니다. 이러한 경고는 대부분의 경우에는 수행할 수 없지만, 임시 개체에서 명확 하지 않은 일부 작업을 수행 하는 위치를 가리킵니다.
