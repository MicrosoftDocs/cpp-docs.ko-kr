...
을 선언 `texture_view` 하는 것은 배열과 연결 된를 선언 하는 것과 비슷합니다 `array_view` .  다음 코드 예제에서는 `texture` 개체 및 이 개체와 연결된 `texture_view` 개체를 여러 개 선언합니다.
