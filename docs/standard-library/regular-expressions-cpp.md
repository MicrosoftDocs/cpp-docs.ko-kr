...
ms.locfileid: "97159754"
---
# <a name="regular-expressions-c"></a>정규식(C++)

C + + 표준 라이브러리는 여러 정규식 문법을 지원 합니다. 이 항목에서는 정규식을 사용할 때 사용할 수 있는 문법 변형에 대해 설명 합니다.

## <a name="regular-expression-grammar"></a><a name="regexgrammar"></a> 정규식 문법

사용할 정규식 문법을 열거형 값 중 하나를 사용 하 여 지정 합니다 `std::regex_constants::syntax_option_type` . 이러한 정규식 문법은 std:: regex_constants에 정의 되어 있습니다.

- `ECMAScript`: JavaScript 및 .NET 언어에서 사용 하는 문법에 가장 가깝습니다.
- `basic`: POSIX 기본 정규식 또는 BRE입니다.
- `extended`: POSIX 확장 된 정규식 또는 ERE입니다.
- `awk`: 이지만 `extended` 인쇄 되지 않는 문자에 대 한 추가 이스케이프를 포함 합니다.
- `grep`: 이지만 `basic` 줄 바꿈 (' \n ') 문자를 사용 하 여 구분할를 구분할 수도 있습니다.
- `egrep`: 이지만 `extended` 줄 바꿈 문자가 구분할를 구분할 수 있습니다.

기본적으로 문법을 지정 하지 않으면 `ECMAScript` 이 가정 됩니다. 문법을 하나만 지정할 수 있습니다.

문법 외에도 몇 가지 플래그를 적용할 수 있습니다.

- `icase`: 일치 하는 경우 대/소문자를 무시 합니다.
- `nosubs`: 표시 된 일치 항목을 무시 합니다 (괄호 안의 식). 대체가 저장 되지 않습니다.
- `optimize`: 더 많은 생성 시간에서 더 빠르게 일치 합니다.
- `collate`: 로캘을 구분 하는 데이터 정렬 시퀀스를 사용 합니다 (예: "[a-z]" 형식의 범위).

0 개 이상의 플래그를 문법과 결합 하 여 정규식 엔진 동작을 지정할 수 있습니다. 플래그만 지정 된 경우 `ECMAScript` 는 문법으로 간주 됩니다.
...
- `expr` 식으로 정의된 집합에도 있는 대상 시퀀스의 문자 또는 데이터 정렬 요소와 일치하는 "[`expr`]" 형식 또는 `expr` 식으로 정의된 집합에 없는 대상 시퀀스의 문자 또는 데이터 정렬 요소와 일치하는 "[^`expr`]" 형식의 *대괄호 식*.
  ...
  - "" 형식의 *문자 범위* 입니다 `ch1` - `ch2` . 닫힌 범위 [`ch1`, `ch2`]의 값으로 표현된 문자를 `expr`로 정의된 집합에 추가합니다.
...
-  \\ `k` 대상 시퀀스의 문자와 일치 하는 "" 형식의 id 이스케이프입니다 `k` .
...
`ECMAScript`, `basic` 및에서 `grep` 요소는 "" 형식에 대 한 *역참조* 일 수도 있습니다. 여기서은 n \\ `dd` `dd` 번째 *캡처 그룹과* 일치 하는 문자 시퀀스와 동일한 대상 시퀀스의 문자 시퀀스와 일치 하는 10 진수 값 N을 나타냅니다. 예를 들어, "(a)\1"는 대상 시퀀스 "aa"와 일치합니다. 그 이유는 첫 번째이자 유일한 캡처 그룹이 초기 시퀀스인 "a"와 일치하고 \1이 마지막 시퀀스인 "a"와 일치하기 때문입니다.

`ECMAScript`에서 요소는 다음 중 하나가 될 수도 있습니다.

- "(?: 하위 *식* )" 형식의 *캡처링 그룹이* 아닙니다. 대상 시퀀스에서 구분 기호 간 패턴과 일치하는 문자 시퀀스와 일치합니다.
...
- "  \\ \\ ", "\A", "\b", "\f", "\n", "\r", "\t" 또는 "\v" 형식의 파일 형식 이스케이프입니다. 대상 시퀀스에서 각각 백슬래시, 경고, 백스페이스, 폼 피드, 줄 바꿈, 캐리지 리턴, 가로 탭, 수직 탭과 일치합니다.

- "" 형식의 *8 진수 이스케이프 시퀀스* \\ `ooo` 입니다. 대상 시퀀스에서 표현이 한 자리, 두 자리 또는 세 자리 8진수 `ooo`로 표현된 값인 문자와 일치합니다.

### <a name="repetition"></a>반복

*긍정 어설션*, *부정 어설션* 또는 *앵커* 이외의 모든 요소 다음에 반복 횟수가 올 수 있습니다. 가장 일반적인 유형의 반복 횟수는 `basic` 및 `grep`의 "\\{`min`,`max`\\}" 또는 "{`min`,`max`}" 형태입니다. 이러한 형태의 반복 횟수 앞에 오는 요소는 해당 요소와 일치하는 시퀀스가 연속적으로 `min`개 이상, `max`개 이하일 경우 일치합니다. 예를 들어 "a"는 대상 시퀀스 " {2,3} aa" 및 대상 시퀀스 "aaa"와 일치 하지만 대상 시퀀스 "a" 또는 대상 시퀀스 "aaaa"와는 일치 하지 않습니다.
...
- "a {2} "는 대상 시퀀스 "aa"와 일치 하지만 대상 시퀀스 "a" 또는 대상 시퀀스 "aaa"와는 일치 하지 않습니다.

- "a"는 대상 시퀀스 {2,} "aa", 대상 시퀀스 "aaa" 등과 일치 하지만 대상 시퀀스 "a"와는 일치 하지 않습니다.

- "a \* "는 대상 시퀀스 "", 대상 시퀀스 "a", 대상 시퀀스 "aa"와 일치 합니다.
...
- "a?" 대상 시퀀스 "" 및 대상 시퀀스 "a"와 일치 하지만 대상 시퀀스 "aa"와는 일치 하지 않습니다.

- "a+"는 대상 시퀀스 "a", 대상 시퀀스 "aa" 등과 일치하지만 대상 시퀀스 ""와는 일치하지 않습니다.

에서 `ECMAScript` 모든 형태의 반복 횟수 다음에는 *non-greedy 반복* 을 지정 하는 문자 '? '가 올 수 있습니다.

### <a name="concatenation"></a>연결

정규식 요소는 *반복 횟수* 유무와 상관없이 연결하여 긴 정규식을 형성할 수 있습니다. 결과 식은 개별 요소와 일치하는 시퀀스 연결인 대상 시퀀스와 일치합니다. 예를 들어 "a {2,3} b"는 대상 시퀀스 "aab" 및 대상 시퀀스 "aaab"와 일치 하지만 대상 시퀀스 "ab" 또는 대상 시퀀스 "aaaab"와는 일치 하지 않습니다.
...
컨트롤 이스케이프 시퀀스는 백슬래시, 문자 'c', 'a' - 'z' 또는 'A' - 'Z' 문자 중 하나의 순서로 구성됩니다. 해당 문자로 이름이 지정되는 ASCII 컨트롤 문자와 일치합니다. 예를 들어 "\ci"는 0x09 값을 가지 므로 대상 시퀀스 "\x09"와 일치 합니다 \<ctrl-i> .
...
- "a \* "는 대상 시퀀스 "aaa"와 일치 하지만 대상 시퀀스 "a"와는 일치 하지 않습니다 \* .

- "a \\ \* "는 대상 시퀀스 "aaa"와 일치 하지 않지만 대상 시퀀스 "a"와 일치 합니다 \* .
...
|`extended`, `egrep`|{' (', ') ', ' {', '. ', ' [', ' \\ ', ' \* ', ' ^ ', ' $ ', ' + ', '? ', ' &#124; '}|
|`awk`|`extended` 더하기 { '"', '/' }|
|`ECMAScript`|식별자에 포함될 수 있는 문자를 제외한 모든 문자. 일반적으로 여기에는 문자, 숫자, ' $ ', ' \_ ' 및 유니코드 이스케이프 시퀀스가 포함 됩니다. 자세한 내용은 ECMAScript 언어 사양을 참조하십시오.|
...
부정 어설션은 해당 내용 이외의 모든 항목과 일치합니다. 대상 시퀀스의 문자는 사용하지 않습니다. 예를 들어 "(! aa) (a \* )"는 대상 시퀀스 "a"와 일치 하 고 캡처 그룹 1을 하위 시퀀스 "a"와 연결 합니다. 대상 시퀀스 "aa" 또는 대상 시퀀스 "aaa"와는 일치하지 않습니다.
...
비캡처 그룹은 정규식 문법에서 내용을 단일 단위로 표시하지만 대상 텍스트에 레이블을 지정하지 않습니다. 예를 들어, "(a) (?: b) \* (c)"는 대상 텍스트 "abbc"와 일치 하 고 캡처 그룹 1을 하위 시퀀스 "a"와 연결 하 고 캡처 그룹 2를 하위 시퀀스 "c"와 연결 합니다.

### <a name="non-greedy-repetition"></a>non-greedy 반복

non-greedy 반복은 패턴과 일치하는 대상 시퀀스의 가장 짧은 하위 시퀀스를 사용합니다. greedy 반복은 가장 긴 항목을 사용합니다. 예를 들어, "(a +) (a \* b)"는 대상 시퀀스 "aaab"와 일치 합니다. non-greedy 반복을 사용할 경우 캡처 그룹 1을 대상 시퀀스의 시작 부분에 있는 시퀀스 "a"와 연결하고 캡처 그룹 2를 대상 시퀀스의 끝에 있는 시퀀스 "aab"와 연결합니다. greedy 일치를 사용할 경우 캡처 그룹 1을 하위 시퀀스 "aaa"와 연결하고 캡처 그룹 2를 하위 시퀀스 "b"와 연결합니다.
...
- ' \* '은 정규식의 첫 번째 문자 또는 정규식의 초기 ' ^ ' 다음에 오는 첫 번째 문자 또는 캡처 그룹의 첫 번째 문자 이거나 캡처 그룹의 초기 ' ^ ' 다음에 오는 첫 번째 문자인 경우를 제외 하 고는 모든 경우에 특별 한 의미가 있습니다.
...
- "(= aa) (a \* )"는 대상 시퀀스 "aaaa"와 일치 하 고 캡처 그룹 1을 하위 시퀀스 "aaaa"와 연결 합니다.

- "(aa) (a \* )"는 대상 시퀀스 "aaaa"와 일치 하 고 캡처 그룹 1을 대상 시퀀스의 시작 부분에 있는 하위 시퀀스 "aa"와 연결 하 고, 캡처 그룹 2를 대상 시퀀스의 끝에 있는 하위 시퀀스 "aa"와 연결 합니다.
...
|"$$"||"$"|
||" \\&"|"&"|
|"$ \` " (달러 기호가 오고 뒤에 오는 따옴표) | | 정규식과 일치 하는 하위 시퀀스 앞에 오는 문자 시퀀스 ( `[match.prefix().first, match.prefix().second)` )|
|"$'"(달러 기호와 작은따옴표)||시퀀스 다음의 정규식과 일치하는 문자 시퀀스(`[match.suffix().first, match.suffix().second)`)|
|"$n"|"\n"|위치에서 캡처 그룹과 일치 하는 문자 시퀀스입니다. `n` 여기서 `n` 은 0에서 9 () 사이의 숫자입니다. `[match[n].first, match[n].second)`|
||"\\\n"|"\n"|
|"$nn"||위치에서 캡처 그룹과 일치 하는 문자 시퀀스입니다. `nn` 여기서 `nn` 은 10에서 99 () 사이의 숫자입니다. `[match[nn].first, match[nn].second)`|
