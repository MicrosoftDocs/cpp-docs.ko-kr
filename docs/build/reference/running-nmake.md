---
title: NMAKE 실행
description: NMAKE를 실행 하는 방법에 대해 알아봅니다.
ms.date: 02/09/2020
helpviewer_keywords:
- targets, building
...
NMAKE 옵션은 다음 표에 설명 되어 있습니다. 옵션 앞에 슬래시 ( `/` ) 또는 대시 ( `-` )가 있고 대/소문자를 구분 하지 않습니다. [`!CMDSWITCHES`](makefile-preprocessing-directives.md)메이크파일의 또는 Tools.ini에서 옵션 설정을 변경 하는 데 사용 합니다.
...
| **D** | 확인 된 각 대상과 종속의 타임 스탬프와 대상이 존재 하지 않는 경우 메시지를 표시 합니다. 메이크파일 디버깅에 사용 되는 **/p** 에 유용 합니다. `!CMDSWITCHES`메이크파일의 일부에 대해 **/d** 를 설정 하거나 해제 하려면를 사용 합니다. |
| **/E** | 환경 변수가 메이크파일 매크로 정의를 재정의 하도록 합니다. |
| **/ERRORREPORT** [ **없음** &#124; **프롬프트** &#124; **QUEUE** &#124; **송신** ] | 더 이상 사용되지 않습니다. [WER (Windows 오류 보고)](/windows/win32/wer/windows-error-reporting) 설정은 보고를 제어 합니다. |
...
| **/G** | 지시문에 포함 된 메이크파일을 표시 `!INCLUDE` 합니다. 자세한 내용은 [메이크파일 전처리 지시문](makefile-preprocessing-directives.md)을 참조 하세요. |
| **/Help**, **/?** | NMAKE 명령줄 구문에 대 한 간략 한 요약을 표시 합니다. |
| **/I** | 모든 명령의 종료 코드를 무시 합니다. 메이크파일의 일부에 대해 **/i** 를 설정 하거나 해제 하려면를 사용 `!CMDSWITCHES` 합니다. 메이크파일의 일부에 대 한 종료 코드를 무시 하려면 대시 ( `-` ) 명령 한정자 또는를 사용 [`.IGNORE`](dot-directives.md) 합니다. 둘 다 지정 된 경우 **/k** 를 재정의 합니다. |
...
| **/R** | 목록을 지우고 `.SUFFIXES` Tools.ini 파일에 정의 되어 있거나 미리 정의 된 유추 규칙 및 매크로를 무시 합니다. |
| **/S** | 실행 된 명령의 표시를 표시 하지 않습니다. 메이크파일의 일부로 표시를 표시 하지 않으려면 **\@** 명령 한정자 또는를 사용 [`.SILENT`](dot-directives.md) 합니다. 메이크파일의 일부에 대해 **/s** 를 설정 하거나 해제 하려면를 사용 `!CMDSWITCHES` 합니다. |
...
**/R** 을 사용 하지 않는 경우 NMAKE는 메이크파일을 읽기 전에 Tools.ini을 읽습니다. 이 메서드는 현재 디렉터리에서 Tools.ini를 먼저 찾은 다음 INIT 환경 변수로 지정 된 디렉터리에서 찾습니다. 초기화 파일의 NMAKE 설정에 대 한 섹션은로 시작 `[NMAKE]` 하며 메이크파일 정보를 포함할 수 있습니다. 숫자 기호 ()로 시작 하는 별도의 줄에 주석을 지정 `#` 합니다.
