...
C + +에서는 오버 로드를 허용 하므로 또는 값을 사용 하 고 반환 하는 **cosh** 의 오버 로드를 호출할 수 있습니다 **`float`** **`long double`** . C 프로그램에서 매크로를 사용 하 여이 함수를 호출 하지 않는 한 \<tgmath.h> **cosh** 는 항상를 사용 하 고 반환 **`double`** 합니다.
