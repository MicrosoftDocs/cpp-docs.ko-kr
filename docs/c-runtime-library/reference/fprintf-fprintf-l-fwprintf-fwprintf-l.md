---
description: Fprintf, _fprintf_l, fwprintf, _fwprintf_l에 대해 자세히 알아보세요.
...
**fprintf** 는 쓴 바이트 수를 반환 합니다. **fwprintf** 는 쓴 와이드 문자 수를 반환 합니다. 이러한 각 함수는 출력 오류가 발생할 때 대신 음수 값을 반환합니다. *스트림* 또는 *형식이* **NULL** 이면 이러한 함수는 [매개 변수 유효성 검사](../../c-runtime-library/parameter-validation.md)에 설명 된 대로 잘못 된 매개 변수 처리기를 호출 합니다. 계속 해 서 실행 하도록 허용한 경우 함수는-1을 반환 하 고 **errno** 를 **EINVAL** 로 설정 합니다. 서식 문자열은 **fprintf_s** 또는 **fwprintf_s** 를 사용할 때와 같이 올바른 형식 지정 문자에 대해 확인 되지 않습니다.
