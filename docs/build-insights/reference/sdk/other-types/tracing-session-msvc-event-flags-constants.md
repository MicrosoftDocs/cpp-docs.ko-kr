...
`TRACING_SESSION_MSVC_EVENT_FLAGS` 상수는 추적 중에 수집할 MSVC 이벤트를 설명하는 데 사용됩니다. 이 상수를 사용하여 [TRACING_SESSION_OPTIONS](tracing-session-options-struct.md) 구조체의 `MsvcEventFlags` 필드를 초기화하세요.
...
| `TRACING_SESSION_MSVC_EVENT_FLAGS_ALL` | 이 플래그는 모든 이벤트를 설정합니다. |
