...
해당하는 [FileOutput](file-output.md) 기본 클래스에서 상속된 멤버와 함께 `ImpLibOutput` 클래스에는 다음 멤버가 포함됩니다.
